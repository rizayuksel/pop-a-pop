...
func _on_body_entered(body):
	if body is RigidBody2D:
		if not is_popped and body.has_method("split_arrow"):
			body.split_arrow()
		pop()
...
func _on_area_entered(area):
	if area is RigidBody2D or area.name.contains("Arrow"):
		if not is_popped:
			...
			if area.has_method("split_arrow"):
				area.split_arrow()
			elif parent and parent.has_method("split_arrow"):
				parent.split_arrow()
		pop()
...
func pop():
	...
	if has_node("Sprite2D"):
		...
		$Sprite2D.modulate = Color("ee799f")
